    ...
    dataDetails.query(
        ...
        (error, results) => { // Changed 'res' to 'results' to prevent overwriting
            if (error) {
                ...
                res.status(200).json({ message: 'subscribe email Footer added successfully' });
            }
        }
    );
}
...
const getdetailss = async (req, res) => {
    dataDetails.query('SELECT * FROM `subscribe` ', (error, results) => {
      if (error) {
  ...
  const getdetailsById = async (req, res) => {
    ...
    dataDetails.query('SELECT * FROM `subscribe` WHERE `id` = ?', [id], (error, results) => {
      if (error) {
...
const updatedetails = async (req, res) => {
    ...
    const sqlQuery = `UPDATE subscribe SET ${setClause.join(', ')} WHERE id = ?`;
    ...
    dataDetails.query(sqlQuery, queryParams, (error, results) => {
        if (error) {
            ...
            if (results.affectedRows === 0) {
                res.status(404).json({ message: 'Subscribe Email not found or no new data to update' });
            } else {
                res.status(200).json({ message: 'Subscribe Email updated successfully' });
            }
        }
    });
};
  ...
  const deletedetails = async (req, res) => {
    const id = req.params.id;
    dataDetails.query('DELETE FROM `subscribe` WHERE `id` = ?', [id], (error, results) => {
      if (error) {
        res.status(500).json({ error: 'Internal Server Error' });
      } else {
        res.status(200).json({ message: 'Subscribe Email deleted successfully' });
      }
    });
  };
